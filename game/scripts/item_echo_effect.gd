...
func configure(id: String, source_player: Node, arena_enemies: Array, definition := {}) -> void:
	...
	match str(effect.get("type", "")):
		"damage_orb":
			...
			damage_per_second = int(effect.get("damagePerSecond", 5))
		"delayed_second_hit":
	...
	match str(effect.get("type", "")):
		"restore_will":
			if is_instance_valid(player):
				player.current_will = mini(player.max_will, player.current_will + int(effect.get("amount", 8)))
		"brief_armor_gain":
			if is_instance_valid(player):
				player.apply_status("bone_memory", float(effect.get("durationSeconds", 4.0)), float(effect.get("armor", 3)))
		"chill_nearby":
			for target in _nearby_enemies():
				target.apply_status("chill", float(effect.get("durationSeconds", 3.0)), float(effect.get("slowPercent", 30)) / 100.0)
		"vulnerable_nearby":
			for target in _nearby_enemies():
				target.apply_status("vulnerable", float(effect.get("durationSeconds", 4.0)), float(effect.get("damageTakenPercent", 20)) / 100.0)
		"bell_pulse":
			_damage_nearby(int(effect.get("damage", 16)), Color(0.55, 0.85, 1.0))
